<template>
    ...
    <div style="margin: 10px;">
        ...
        <pet-modal ref="petModal" v-model:pet="pet" :is-show-insert="isShowInsert" @insert="callCreate"
            @update="callUpdate">
        </pet-modal>
    </div>
</template>
...
const isShowInsert = ref(true);  // 是否顯示新增按鈕
...
function openModal(action, id) {
    if (action === 'insert') {
        isShowInsert.value = true;  // 顯示新增按鈕
        pet.value = {};  // 清空當前動物資料
    } else if (action === 'edit') {
        isShowInsert.value = false;  // 隱藏新增按鈕，顯示更新按鈕
